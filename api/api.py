...
from mlcc.common.common import get_date_from_string, get_meal_type_by_name, get_meal_type_by_value, is_quantity_valid, \
    guess_quantity
...
def create_food(food_id: str, food_item: FoodModel):
    if food_id != food_item.name:
        raise HTTPException(status_code=400, detail=f"food_id ({food_id} "
                                                    f"and food name ({food_item.name}) do not match")
    calories = food_item.nutrition_data.calories
    quantity = food_item.nutrition_data.quantity
    unit_type = None
    for ut in UnitType:
        if ut.get_value() == quantity.unit_type:
            unit_type = ut
            break
    if unit_type is None:
        raise HTTPException(status_code=400, detail=f"Invalid unit type ({quantity.unit_type})")

    engine.get_food_data().add(name=food_id, calories=calories, quantity=quantity.value,
                               unit_type=unit_type, unit_symbol=quantity.unit_symbol)
    food = engine.food_data.get_food_by_name(food_id)
    if food is None:
...
@app.post("/types/unit/guessed_quantity")
def valid_unit_type(quantity_item: QuantityModel):
    unit_type = None
    for ut in UnitType:
        if ut.get_value() == quantity_item.unit_type:
            unit_type = ut
            break
    if unit_type is None:
        raise HTTPException(status_code=400, detail=f"Invalid unit type ({quantity_item.unit_type})")

    if not is_quantity_valid(quantity_item.value, unit_type, quantity_item.unit_symbol):
        raise HTTPException(status_code=400, detail=f"Invalid quantity ({quantity_item.value} "
                                                    f"{quantity_item.unit_symbol} with type {quantity_item.unit_type})")
    return {"guessed_quantity": guess_quantity(quantity_item.value, unit_type, quantity_item.unit_symbol)}


@app.get("/data")
def guess_valid_quantity(show: Union[str, None] = None):
    if show is None or show != "all":
...
def add_food_to_meal(day_date: str, meal_type: Union[str, int], food_id: str, q: Union[str, None] = None):
    actual_date = get_date_from_string(day_date)
    if actual_date is None:
        raise HTTPException(status_code=400, detail="Invalid date (YYYY-MM-DD expected)")
